...
char* serverlogfile = "./log/serverlog";
...
//listen to the socket by creating a connection queue, then wait for clients
    ...
    log_message(serverlogfile,"Server started - waiting for connections\n");
    while(1) {
        if(quit==0) {
            ...
            printf("%s\n","Received request...");
            log_message(serverlogfile,"Received request...\n");
        ...
        if (childpid < 0) {
            ...
            log_message(serverlogfile,"Child created for dealing with client requests\n");
            uint32_t crc = 0;
            uint8_t new_hash[5];
            while (((n = recv(connfd, buf, MAXLINE-1,0)) > 0) && (quit == 0)) {
                buf[n-1] = '\0';
                sprintf(logbuf,"String received from client: %s\n", buf);
                log_message(serverlogfile,logbuf);
                printf("%s", logbuf);
